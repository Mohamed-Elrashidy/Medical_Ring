import 'package:flutter/material.dart';
import 'package:flutter_bloc/flutter_bloc.dart';
import 'package:flutter_screenutil/flutter_screenutil.dart';
import 'package:flutter_svg/svg.dart';
import 'package:medical_ring_app/Core/helpers/extension.dart';
import 'package:medical_ring_app/core/theming/theming_bloc/themeing_cubit.dart';
import 'package:medical_ring_app/core/widgets/vertical_space.dart';

import '../theming/my_app_icons.dart';
import '../theming/text_styles.dart';

import 'horizontal_space.dart';

class MyAppAppBar extends StatelessWidget {
  MyAppAppBar({required this.title, super.key, this.showBack = true});
  String title;
  bool showBack;
  late ThemingCubit themingCubit;
  @override
  Widget build(BuildContext context) {
    themingCubit = BlocProvider.of<ThemingCubit>(context);
    return Column(
      children: [
        const VerticalSpace(height: 16),
        Row(children: [
          (showBack)
              ? Transform(
                  alignment: Alignment.center,
                  transform:
                      Matrix4.rotationY(themingCubit.isArabic() ? 3.14 : 0),
                  child: InkWell(
                    onTap: () {
                      context.pop();
                    },
                    child: SvgPicture.asset(MyAppIcons.backIcon.icon,
                        width: 14.w, height: 14.h),
                  ))
              : Container(),
          (showBack)? const HorizontalSpace(width: 16):Container(),
          Text(title, style: MyAppTextStyle.title),
        ]),
      ],
    );
  }
}
